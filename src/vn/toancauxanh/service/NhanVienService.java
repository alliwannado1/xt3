...
public final class NhanVienService extends BasicService<NhanVien> {
	...
	public JPAQuery<NhanVien> getTargetQueryNhanVien() {
		...
		q.where(QNhanVien.nhanVien.tenDangNhap.ne("admin"));
	...
	@Command
	public void login(@BindingParam("tenDangNhap") final String tenDangNhap, @BindingParam("password") final String password) {
		NhanVien nhanVien = new JPAQuery<NhanVien>(em()).from(QNhanVien.nhanVien)
				...
				.where(QNhanVien.nhanVien.tenDangNhap.eq(tenDangNhap)).fetchFirst();
		BasicPasswordEncryptor encryptor = new BasicPasswordEncryptor();
		if (nhanVien != null && encryptor.checkPassword(password.trim() + nhanVien.getSalkey(), nhanVien.getMatKhau())) {
			String cookieToken = nhanVien.getCookieToken(System.currentTimeMillis() + TimeUnit.MILLISECONDS.convert(6, TimeUnit.HOURS));
